...
function updateConstellation(constellation){
	document.getElementById("resultWeight").innerHTML = (constellation.getWeight() * 0.25);
	document.getElementById("resultAcceleration").innerHTML = (constellation.getAcceleration() * 0.25);
	document.getElementById("resultOnRoadTraction").innerHTML = (constellation.getOnRoadTraction() * 0.25);
	document.getElementById("resultOffRoadTraction").innerHTML = (constellation.getOffRoadTraction() * 0.25);
	document.getElementById("resultMiniTurbo").innerHTML = (constellation.getMiniTurbo() * 0.25);
	document.getElementById("resultGroundSpeed").innerHTML = (constellation.getGroundSpeed() * 0.25);
	document.getElementById("resultWaterSpeed").innerHTML = (constellation.getWaterSpeed() * 0.25);
	document.getElementById("resultAntiGravitySpeed").innerHTML = (constellation.getAntiGravitySpeed() * 0.25);
	document.getElementById("resultAirSpeed").innerHTML = (constellation.getAirSpeed() * 0.25);
	document.getElementById("resultGroundHandling").innerHTML = (constellation.getGroundHandling() * 0.25);
	document.getElementById("resultWaterHandling").innerHTML = (constellation.getWaterHandling() * 0.25);
	document.getElementById("resultAntiGravityHandling").innerHTML = (constellation.getAntiGravityHandling() * 0.25);
	document.getElementById("resultAirHandling").innerHTML = (constellation.getAirHandling() * 0.25);
	document.getElementById("resultInvincibility").innerHTML = (constellation.getInvincibility() * 0.25);
}
...
function searchBestSumedWeightedValue(vector){
	...
	let chosenVehicle = document.getElementById("vehicle").value;
	
	for (let i = 0; i < drivers.length; i++) {
		for (let j = 0; j < bodys.length; j++) {
			for (let k = 0; k < tires.length; k++) {
				for (let l = 0; l < gliders.length; l++) {
					let tempConstellation = new constellation(drivers[i],bodys[j],tires[k],gliders[l]);
					let valueOfTempConstellation = getWeightedSumedValue(tempConstellation,vector);
					if( valueOfTempConstellation > SumedValue ){
						if( chosenVehicle == tempConstellation.body.type || chosenVehicle == "all"){
							SumedValue = valueOfTempConstellation;
							bestResult = tempConstellation;
						}
					}
				}
			}
...
function startSearch(){
	...
	updateConstellation(bestConstellation);
	}

// Function to change the info-filed about vehicles
function refreshInfo() {

	const texts = {
		all: "Bei der Suche werden alle Fahrzeugtypen berücksichtigt.",
		Kart: "Karts are the main vehicles in the Mario Kart series.",
		Outward: "Outward drifting bikes are a class of bikes in the Mario Kart series that are able to drift normally.",
		Inward: "Inward drifting bikes are a class of bikes in the Mario Kart series that drift in a unique way, committing to the turn instead of actually 'drifting'. When the player attempts to manually drift or autodrift, they handle in a significantly different way from every other vehicle class ",
		ATV: "All-Terrain Vehicles, abbreviated as ATVs (known as Quad Bikes or just Quads in the British English version)."
	};

	const vehicle = document.getElementById("vehicle").value;
	const infoText = texts[vehicle] || "Bitte wähle eine gültige Option aus.";
